<template>
  <div class="outpatient-review">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>门诊病历质控 - 待审核列表</span>
        </div>
      </template>
      ...
      >
        ...
        <el-table-column prop="doctor.realName" label="学生" width="120" />
      ...
      <el-form :model="reviewForm" label-width="100px">
        ...
        <el-form-item label="质控结果" required>
          <el-radio-group v-model="reviewForm.result">
            ...
            <el-radio label="rejected">不通过（需整改）</el-radio>
          </el-radio-group>
        </el-form-item>
...
import { getPendingReviewRecords, reviewRecord } from '@/api/outpatient'
...
const fetchRecords = async () => {
  ...
  try {
    ...
    recordList.value = response.data?.list || response.list || []
...
const confirmReview = async () => {
  ...
  try {
    ...
    ElMessage.success(reviewForm.result === 'passed' ? '审核通过' : '已退回修改')
